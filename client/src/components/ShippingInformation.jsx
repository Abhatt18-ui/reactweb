...
const ShippingInformation = () => {
    ...
    const [formStateChaned,setFormStatechanged]= useState(false)

    const setErrorState = (input, data)=>{
      if(!input){
        dispatch(setShippingAddress(data));
      }
      if((!formStateChaned && !input)||(formStateChaned && input)){
        return;
      }else{
...
<VStack as='form'>
    <FormControl onChange={
     ...
     <TextFiled name='address' placeholder='Street Address' label='Street Address'/> 
     <Flex>
        <Box flex='1' mr='10'>
        <TextFiled name='postalCode' placeholder='Postal Code' label='Postal Code' 
        type='number'/> 
        </Box>
        <Box flex='2'>
        <TextFiled name='city' placeholder='City' label='City'/> 
        </Box>
     </Flex>  
     <TextFiled name='country' placeholder='Country' label='Country'/> 
    </FormControl>
